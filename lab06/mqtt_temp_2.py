...
# MQTT Configuration
id = 'willasp'  # Unique ID for this device
client_name = id + 'temperature_client'
client_telemetry_topic = id + '/telemetry'
server_command_topic = id + '/commands'  # Topic for receiving commands

# Create MQTT client and connect
mqtt_client = mqtt.Client(client_id=client_name, protocol=mqtt.MQTTv5, callback_api_version=mqtt.CallbackAPIVersion.VERSION1)
mqtt_client.connect('test.mosquitto.org')
mqtt_client.loop_start()
print("MQTT connected!")
print(f"Publishing to topic: {client_telemetry_topic}")
print(f"Subscribing to commands on: {server_command_topic}")

# Define callback for command messages
def handle_command(client, userdata, message):
    ...
    try:
        ...
        if 'led_on' in payload:
            if payload['led_on']:
                led.on()
                print("LED turned ON by server command")
            else:
                led.off()
                print("LED turned OFF by server command")
    ...
    try:
        while True:
            ...
            if celsius is not None:
                ...
                print(f"Sent telemetry to {client_telemetry_topic}: {telemetry}")
                
                # Note: LED control is now handled by commands from the server
                # The following local LED control is now redundant
                # But kept for fallback in case server is not responding
                if celsius > 25:
                    led.on()
                    print("Temperature > 25°C, LED ON (local control)")
                else:
                    led.off()
                    print("Temperature ≤ 25°C, LED OFF (local control)")
            else:
